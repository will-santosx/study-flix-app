...
const inputSearchMovie = document.getElementById(
  "search-movie"
) as HTMLInputElement;
const buttonSearchMovie = document.getElementById(
  "button-search"
) as HTMLButtonElement;
const movieContainer = document.querySelector(".movies") as HTMLDivElement;
const centerTitle = document.getElementById(
  "center-home-button"
) as HTMLButtonElement;
...
centerTitle.addEventListener("click", function () {
  axios
    .get(
      `https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=${API_KEY}&language=pt-BR&page=1&vote_count.gte=100`
    )
    .then((response) => {
    ...
    movieElement.innerHTML = `
      <div class="movie">
        <div class="movie-info">
          ...
          <span class="movie-info-year">${movie.release_date.substring(
            0,
            4
          )}</span>
        </div>
